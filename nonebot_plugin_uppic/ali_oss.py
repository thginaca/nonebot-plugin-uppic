...
endpoint = uppic_endpoint
# 存储空间名称。
bucket = uppic_bucket
# 填写Bucket所在地域。以华东1（杭州）为例，Region填写为cn-hangzhou。
region = uppic_region
# 填写RAM用户的Access Key ID和Access Key Secret
access_key_id = uppic_oss_access_key_id
access_key_secret = uppic_oss_access_key_secret
...
class OSSUploaderV2:
    ...
    async def delete_file(self, oss_key: str):
        """
        删除OSS中的指定文件
        ...
        try:
            ...
            if result.status_code != 204:
                logger.warning(f"删除OSS文件失败: {oss_key} (状态码: {result.status_code})")
        except Exception as e:
            logger.warning(f"删除OSS文件异常: {oss_key} (错误: {e})")
