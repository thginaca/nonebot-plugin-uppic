...
- **权限控制**：超级用户可跨群配置上传权限
- **自动压缩**：仅对超过 1MB 的静态图压缩到 1MB 以内；1MB 以下图片原样保存不压缩，避免质量损失
- **动图保护**：动画 GIF 保持原图不压缩，避免颜色损坏与帧错乱
...
| 指令 | 说明 |
|------|------|
| `删除分类 <指令名>` | 删除整个分类（文件夹+数据库+配置+关联别名），需二次确认 |

删除时会先显示分类信息和图片数量，发送「确认」后执行删除，支持别名触发。
...
功能：浏览图片、拖拽上传、一键删除图片、删除整个分类、自动刷新。
...
- 仅超过 1MB 的静态图会被压缩，1MB 以下原样保存；动画 GIF 始终保持原图
- 运行期间请勿手动修改图片文件夹
- 建议合理控制图片数量
...
| **添加** | 下载 → 压缩（仅 >1MB 静态图；动图跳过） → pHash 去重 → 保存 → 写入数据库 |
